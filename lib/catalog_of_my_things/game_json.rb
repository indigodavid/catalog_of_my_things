  ...
  def load_games
    ...
    if File.exist?(file) && File.read(file) != ''
      JSON.parse(File.read(file)).each do |game|
        ...
        game_author = @authors.find { |author| game['author'].id == author.id }
        obj.add_author(game_author)

        game_genre = @genres.find { |genre| game['genre'].id == genre.id }
        obj.add_genre(game_genre)

        game_label = @labels.find { |label| game['label'].id == label.id }
